...
@section('content')
    <div class="container">
        <div class="container-body">
            <div class="row">
                <div class="col-sm-3">
                    <form action="{{route('locataire.emissions', [Request::segment(2)])}}" method="POST" class="form w-100">
                        @csrf
                        <select name="substance" id="substance">
                            @forelse ($substances as $substance)
                                <option value="{{$substance->id}}">{{$substance->libelle}}</option>
                            @empty
                                <option value=""></option>
                            @endforelse
                        </select>
                ...
                @else
                    <div class="card">
                      <div class="card-body">
                        <p class="card-text">
                            Sélectionnez une substance afin de voir les stats de son émission :)
                        </p>
                      </div>
                    </div>
                @endif
